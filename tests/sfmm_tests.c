...
Test(sf_memsuite_student, realloc_smaller_block_splinter_free, .init = sf_mem_init, .fini = sf_mem_fini){

	void *x  = sf_malloc(4096);
	cr_assert_not_null(x);

	void *y = sf_realloc(x, 4065);
	cr_assert(x == y, "Not equal");

	sf_free(y);

	free_list *fl = &seg_free_list[find_list_index_from_size(2*PAGE_SZ)];
	cr_assert_not_null(fl->head, "No block in expected free list!");
	cr_assert(fl->head->header.allocated == 0, "Allocated bit is set!");
	cr_assert(fl->head->header.block_size << 4 == 2*PAGE_SZ, "Free block size not what was expected!");
}

Test(sf_memsuite_student, malloc_one_less_than_four_pages, .init = sf_mem_init, .fini = sf_mem_fini){

	void *x = sf_malloc(16383);
	cr_assert_null(x);
	...
	cr_assert(seg_free_list[3].head -> header.block_size << 4 == 4*PAGE_SZ, "Free size block should be 16384");
}

Test(sf_memsuite_student, reallocing_larger_size_value_and_smaller_size_value, .init = sf_mem_init, .fini = sf_mem_fini){
	double *x = sf_malloc(sizeof(double));
	*x = 051317;
	double xValue = *x;
	double *y = sf_realloc(x, sizeof(double)*2);
	cr_assert(xValue == *y, "Value of realloced block changed");

	char *c = sf_realloc(y, sizeof(char));
	cr_assert(*c != xValue, "char should hold differnt value");
}

Test(sf_memsuite_student, mallocing_less_than_4_pages, .init = sf_mem_init, .fini = sf_mem_fini){
...
Test(sf_memsuite_student, checking_for_padding_and_block_size, .init = sf_mem_init, .fini = sf_mem_fini){

	void *x = sf_malloc(4079);
	cr_assert(((sf_header*)(x - 8)) -> padded == 1);
	cr_assert(((sf_header*)(x - 8)) -> block_size << 4 == 4096);
	cr_assert(((sf_header*)(x - 8)) -> allocated == 1);
	...
	void *y = sf_realloc(x, 12288);
	cr_assert_null(y);

	cr_assert_null(seg_free_list[0].head);
	cr_assert_null(seg_free_list[1].head);
	cr_assert_null(seg_free_list[2].head);
	cr_assert(seg_free_list[3].head -> header.block_size << 4 == 3 *PAGE_SZ, "Free size block should be 12288");
}
